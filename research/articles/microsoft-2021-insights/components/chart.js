...
const Chart = ({ data, field, domain, ticks, bandwidth, log = false }) => {
  ...
  if (log) {
    ...
    xTicks = scaleLog()
      .domain(domain)
      .range([(margin / width) * 100, ((width - margin) / width) * 100 - margin])
      .clamp(true)
  } else {
    x = scaleLinear()
      .domain(domain)
  ...
  return (
    ...
    >
      <Box sx={{position: 'relative', display: 'block', width: '100%', height: height}}>
      <Box as='svg' sx={{position: 'absolute', top: '0px'}} width='100%' height={height} viewBox={`0 0 100 ${height}`} preserveAspectRatio='none'>
        {categories.map((c, i) => {
          const subset = data
            .filter((d) => d.tags[0] == c)
      ...
      <Box as='svg' sx={{position: 'absolute', top: 0}} width='100%' height={height} preserveAspectRatio='none'>
        {categories.map((c, i) => {
          const subset = data
            .filter((d) => d.tags[0] == c)
            .map((d) => Math.min(Math.max(d[field], domain[0]), domain[1]))
